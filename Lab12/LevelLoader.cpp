	...
	if (actorValue.IsObject())
	{
		...
		if (type == "Block")
		{
            ...
            if(GetBoolFromJSON(actorValue, "mirror", mirror)){
                block->SetIsMirror(mirror);
            }
            bool rotates = false;
            if(GetBoolFromJSON(actorValue, "rotates", rotates)){
                block->SetIsRotate(rotates);
            }
		}
		else if (type == "Player")
		{
        ...
        {
            Checkpoint* check = new Checkpoint(game, parent);
            check->SetActive();
            actor = check;
            game->AddCheckpoint(check);
            
            std::string level;
            if (GetStringFromJSON(actorValue, "level", level))
            {
                check->SetLevelString(level);
            }
            
            std::string txt;
            if (GetStringFromJSON(actorValue, "text", txt))
            {
                check->SetText(txt);
            }
        }
        else if(type == "LaserMine")
        {
            ...
            float interpTime;
            float pauseTime;
            if (GetQuaternionFromJSON(actorValue, "startQ", qStart)){
                sc->SetStart(qStart);
            }
            if (GetQuaternionFromJSON(actorValue, "endQ", qEnd)){
                sc->SetEnd(qEnd);
            }
            if (GetFloatFromJSON(actorValue, "interpTime", interpTime)){
                sc->SetInterp(interpTime);
            }
            if (GetFloatFromJSON(actorValue, "pauseTime", pauseTime)){
                sc->SetPause(pauseTime);
            }
