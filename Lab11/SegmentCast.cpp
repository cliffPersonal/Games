...
{
	bool outside = FuzzyLessThan(point.x, min.x) ||
		FuzzyLessThan(point.y, min.y) ||
		FuzzyLessThan(point.z, min.z) ||
		FuzzyLessThan(max.x, point.x) ||
		FuzzyLessThan(max.y, point.y) ||
		FuzzyLessThan(max.z, point.z);
...
bool Intersect(const LineSegment& l, const CollisionComponent* cc, float& outT, Vector3& outNorm)
{
	...
	// Test the x planes
	TestSidePlane(l.mStart.x, l.mEnd.x, min.x, Vector3::NegUnitX,
		tValues);
	TestSidePlane(l.mStart.x, l.mEnd.x, max.x, Vector3::UnitX,
		tValues);
	// Test the y planes
	TestSidePlane(l.mStart.y, l.mEnd.y, min.y, Vector3::NegUnitY,
		tValues);
	TestSidePlane(l.mStart.y, l.mEnd.y, max.y, Vector3::UnitY,
		tValues);
	// Test the z planes
	TestSidePlane(l.mStart.z, l.mEnd.z, min.z, Vector3::NegUnitZ,
		tValues);
	TestSidePlane(l.mStart.z, l.mEnd.z, max.z, Vector3::UnitZ,
		tValues);
...
bool SegmentCast(const std::vector<class Actor*>& actors, const LineSegment& l, CastInfo& outInfo, Actor* ignoreActor)
{
	bool collided = false;
